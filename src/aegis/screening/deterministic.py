...
RULES: tuple[_SignalRule, ...] = (
    ...
    _SignalRule(
        "urgency",
        _compile(r"\b(?:urgent(?:ly)?|immediately|right\s+away|act\s+now|final\s+warning|today\s+only|within\s+\d+\s+(?:minutes?|hours?))\b"),
        EvidenceStrength.MODERATE,
        "Urgency language was detected.",
    ),
    _SignalRule(
        "account_threat",
        _compile(r"\b(?:account\s+(?:will\s+be|has\s+been|is)\s+(?:suspended|locked|disabled|closed|limited|compromised)|(?:suspend|lock|disable)\s+.{0,30}?\baccount)\b"),
        EvidenceStrength.MODERATE,
        "Language threatening an account consequence was detected.",
    ),
    _SignalRule(
        "identity_impersonation",
        _compile(r"\b(?:this\s+is|i\s+am|we\s+are)\s+(?:from|with)\s+(?:your\s+)?(?:bank|support|security\s+team|microsoft|google|amazon|paypal|government|police|tax\s+department)\b"),
        EvidenceStrength.MODERATE,
        "A claim of authority or organizational identity was detected.",
    ),
    _SignalRule(
        "suspicious_instruction",
        _compile(r"\b(?:disable\s+(?:security|antivirus|defender)|install\s+(?:this|the)\s+(?:app|application)|download\s+(?:and\s+)?(?:run|open)|enable\s+(?:macros?|unknown\s+sources?))\b"),
        EvidenceStrength.STRONG,
